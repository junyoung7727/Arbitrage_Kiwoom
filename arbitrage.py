...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        spot_codes = self.spot_future_pairs.keys()
    ...
    def check_arbitrage_opportunity(self, f_code):
        print(f"Checking arbitrage opportunity for: {f_code}")

        # Find matching pair for the given code (either spot or future)
        matching_pair = None
        spot_code = self.spot_future_match[f_code]
        matching_pair = self.spot_future_pairs[spot_code]

        print(spot_code,matching_pair)
        print('------------')
        # Check if both spot and future prices are available
        spot_price_info = self.spot_prices[spot_code]
        future_price_info = self.future_prices[spot_code]
        print(self.spot_prices, self.future_prices)

        if spot_price_info is not None and future_price_info is not None:
            self.execute_arbitrage(matching_pair, spot_price_info, future_price_info)
        else:
            print(f"Spot or future prices not available for {f_code}")

    def execute_arbitrage(self, pair, spot_price_info, future_price_info):
        ...
        days_to_expiration = self.get_future_expiration_date(spot_code)
        T = days_to_expiration / 365.0

        # Get dividend yield
        dividend_yield = self.dividend_yields.get(spot_code, 0.0)

        risk_free_rate = self.get_future_interest_rate(spot_code)
        ...
        buy_arbitrage = theoretical_buy_price - future_bid # 매수차익 = 선물매도가 - 선물매수이론가
        sell_arbitrage = future_ask - theoretical_sell_price  # 매도차익 = 선물매수가 - 선물매도이론가
        ...
        if buy_arbitrage > threshold:
            ...
            # Execute buy arbitrage: Buy future at ask price, sell spot at bid price
            ...
            # Execute sell arbitrage: Sell future at bid price, buy spot at ask price
    ...
    def register_real_time_data_spot(self, codes):
        ...
        self.dynamicCall("SetRealReg(QString, QString, QString, QString)", screen_no, codes_string, fid_list, real_type)
    ...
    def register_real_time_data_future(self, codes):
        ...
        print(f"실시간 데이터 등록 완료(선물): {'111VB000'}")
    ...
    def get_all_futures_info(self):
        ...
        for item in basis_asset_list:
            ...
            spot_info.append(code_name_pair)
        ...
        #risk_free_rate = self.get_future_interest_rate(base_asset_code)
        ...
        for item in items:
            ...
            if len(parts) >= 2:
                ...
                if 'F' in product:
                    ...
                    for asset_pair in spot_info:
                        spot_code = asset_pair[0]
                        spot_name = asset_pair[1].strip()
                        ...
                        if spot_name.lower() == asset_name.lower():
                            ...
                            self.spot_future_match[future_code] = spot_code[3:9]
...
class KiwoomApp(QMainWindow):
    def __init__(self):
        ...
        self.timer.start(50000)  # 1초마다 업데이트
    ...
    def update_table(self):
        try:
            ...
            for row, pair in enumerate(self.kiwoom.spot_future_pairs):
                spot_code = pair['spot_code'][3:9]
                future_code = pair['future_code']
                ...
                days_to_expiration = self.kiwoom.get_future_expiration_date(spot_code)
                ...
                theoretical_sell_price = self.kiwoom.calculate_theoretical_future_price(
                    spot_price_info['ask'], risk_free_rate, dividend_yield, days_to_expiration / 365.0
                )
                theoretical_buy_price = self.kiwoom.calculate_theoretical_future_price(
                    spot_price_info['bid'], risk_free_rate, dividend_yield, days_to_expiration / 365.0
                )
